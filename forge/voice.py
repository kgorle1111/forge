...
def make_voice_ask(base_ask, base_emit):
    ...
    def ask(prompt: str) -> str:
        ...
        while True:
            hint = ("[voice] Enter to talk (Enter again to stop), or type an "
                    "answer here:")
            user = input(hint + "\n> ")
            if user.strip():
                return user
            ...
            choice = input("[voice] y=submit, e=edit, r=redo, t=type: ").strip().lower()
            if choice == "y" and transcript:
                return transcript
            if choice == "e":
                edited = input(f"[voice] edit> ").strip()
                if edited:
                    return edited
            if choice == "t":
                typed = input("> ")
                if typed.strip():
                    return typed
            # anything else -> loop back to redo
